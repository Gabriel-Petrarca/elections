...
function Marketing() {
  const [MC_Candidates, setMC_Candidates] = useState([]);
  ...
  const fetchCandidates = () => {
    fetch(`/MC_candidates?_t=${timestamp}`)
      .then((response) => response.json())
      .then((data) => {
        console.log('Fetched candidates data:', data);
        setMC_Candidates(data.MC_candidates);
      })
  ...
  return (
    <div className="MC">
      <div className="vote_for_name">
        <h1>Vote for VP of Marketing Communication</h1>
        <div className="candidates">
          {MC_Candidates.map((candidate, index) => (
        ...
        <button
          ...
          onClick={() => submitVote('MC', userEmail, optionChosen)}
        >
          Submit Vote
        </button>
      </div>
