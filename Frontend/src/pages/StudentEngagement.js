...
function StudentEngagement() {
  const [SE_Candidates, setSE_Candidates] = useState([]);
  ...
  useEffect(() => {
    const fetchVotingStatus = async () => {
      try {
        ...
        if (!data.voting_status.SE) {
          // Voting for SE is closed, redirect to the home page
          navigate('/');
        } else {
  ...
  const fetchCandidates = () => {
    fetch(`/SE_candidates?_t=${timestamp}`)
      .then((response) => response.json())
      .then((data) => {
        console.log('Fetched candidates data:', data);
        setSE_Candidates(data.SE_candidates);
      })
  ...
  return (
    <div className="SE">
      <div className="vote_for_name">
        <h1>Vote for VP of Student Engagement</h1>
        <div className="candidates">
          {SE_Candidates.map((candidate, index) => (
        ...
        <button
          ...
          onClick={() => submitVote('SE', userEmail, optionChosen)}
        >
          Submit Vote
        </button>
      </div>
