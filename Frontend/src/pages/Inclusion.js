...
function Inclusion() {
  const [IandB_Candidates, setIandB_Candidates] = useState([]);
  ...
  const fetchCandidates = () => {
    fetch('/IandB_candidates')
      ...
      .then((data) => {
        ...
        setIandB_Candidates(data.IandB_candidates);
      })
  ...
  return (
    <div className="IandB">
      <div className="vote_for_name">
        <h1>Vote for VP of Inclusion & Belonging</h1>
        <div className="candidates">
          {IandB_Candidates.map((candidate, index) => (
        ...
        <button
          ...
          onClick={() => submitVote('IandB', userEmail, optionChosen)}
        >
          Submit Vote
        </button>
      </div>
